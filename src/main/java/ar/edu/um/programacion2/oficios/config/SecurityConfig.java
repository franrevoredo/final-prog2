...
class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/administradors/**", "/admpanel/**", "/categorias/**", "/disponibilidads/**", "/localidads/**", "/personae/**", "/clientes/list").hasAuthority("ROLE_ADMINISTRADOR");
		http.authorizeRequests().antMatchers("/servicios/create-form", "/calificacionprestadors/**").hasAuthority("ROLE_PRESTADOR");
		http.authorizeRequests().antMatchers("/calificacionclientes/**").hasAuthority("ROLE_CLIENTE");
		http.authorizeRequests().antMatchers("/buscar/**", "/myprofile", "/clientes/**").authenticated().and().formLogin().loginPage("/login").and().logout().logoutUrl("/logout").logoutSuccessUrl("/");
