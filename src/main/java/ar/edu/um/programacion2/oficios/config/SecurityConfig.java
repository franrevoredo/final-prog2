...
class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/administradors/**", "/admpanel/**", "/categorias/create-form", "/disponibilidads/create-form", "/localidads/create-form", "/categorias/list", "/disponibilidads/list", "/localidads/list", "/personae/list", "/clientes/list", "/prestadors/list").hasAuthority("ROLE_ADMINISTRADOR");
		...
		http.authorizeRequests().antMatchers("/public/**", "/js/**", "/webjars/**", "/", "/clientes/create-form", "/prestadors/create-form", "/ver-servicio/**", "/categorias/s2", "/disponibilidads/s2", "/localidads/s2").permitAll().anyRequest().anonymous();
		http.csrf().disable();
	}
}
